...
# ---------------------------------------------------------------------------
# One format per major: derivatives of an earlier format are refused by name
# ---------------------------------------------------------------------------

def test_derivatives_of_an_earlier_format_are_refused_by_name(dbuilt):
    """2.0.0 reads and writes one format. A v2 pair (the one a stranger
    downloaded under 1.x) is not read at the wrong widths and not
    called unknown: the refusal names the tag and the release that
    reads it."""
    import shutil
    tmp, _graph, index, derived, _txids, _locks = dbuilt
    old = os.path.join(tmp, "derived_v2_label")
    shutil.copytree(derived, old)
    for name in (dv.STATE_NAME, dv.MANIFEST_NAME):
        path = os.path.join(old, name)
        doc = json.load(open(path))
        doc["format"] = "outpoint-derived-v2"
        if "identity" in doc:
            doc["identity"]["format"] = "outpoint-derived-v2"
        with open(path, "w") as f:
            json.dump(doc, f)
    idx = oi.Index(index)
    try:
        for road in (lambda: dv.Derived(old, idx),
                     lambda: dv.run_verify(old),
                     lambda: dv.run_build(index, old)):
            try:
                road()
                fail("derivatives of an earlier format were read or grown")
            except dv.OutpointError as e:
                check("outpoint-derived-v2" in str(e)
                      and "earlier format" in str(e), f"unexpected: {e}")
    finally:
        idx.close()
    print("ok  derivatives of an earlier format: refused by name, read "
          "with the release that wrote them")
