...
| `scan/` | the reveal archive's scan, one block at a time: block → the sorted, reduced run of every record category (count + sha256), the counters moved, the header facts (Merkle root, weight, digest of the txids) | ten synthetic blocks with their bytes included, covering every spend shape the suite knows; 200 real blocks from five eras, referenced by height and hash (fetched from any node; `tests/test_conformance.py` runs them when `NODSIG_VECTOR_NODE` names one). Regenerated by `tools/make_scan_vectors.py` from the reference's own per-block body, so the target of a native kernel is what the scan does, not a restatement |
